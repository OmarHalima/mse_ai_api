...
const parseVqdHash = async (userAgent, hash) => {
    let currentWindow;
    let currentDocument;
    ...
    return new Function("window", "navigator", "document", `return ${atob(hash)}`)(currentWindow, currentNavigator, currentDocument);
};
const parseDDGHash = (html) => {
...
const getVqdHash = async (request) => {
    if (!request)
        request = fetch;
    const payload = setupFetch({ method: "GET" });
    const response = await request(`https://duckduckgo.com/duckchat/v1/status`, payload);
    if (!response.ok)
        throw new Error("Failed get Vqd");
    const vqdHash = response.headers.get("X-Vqd-Hash-1") || "";
    const ua = payload.headers && payload.headers["user-agent"];
...
const getModels = async (request) => {
    if (!request)
        request = fetch;
    const payload = setupFetch({ method: "GET" });
    const html = await (await request("https://duckduckgo.com/?q=DuckDuckGo+AI+Chat&ia=chat&duckai=1", payload)).text();
    const { DDG_FE_CHAT_HASH } = parseDDGHash(html);
    const js = await (await request(`https://duckduckgo.com/dist/wpm.chat.${DDG_FE_CHAT_HASH}.js`, payload)).text();
    const match = js.match(/\$e=\[([^\]]+)\],et=\[(.*)\],tt=/);
    if (!match || !match[1] || !match[2])
        throw new Error("Failed get models");
    const supportedTools = new Function(`return [${match[1]}]`)();
    const models = new Function(`return [${match[2].replace(/v\.(Free|Internal|Plus)/g, (_, w1) => `"${w1.toUpperCase()}"`).replace(/\$e/g, JSON.stringify(supportedTools))}]`)();
    const availableModels = models
        .filter((model) => model.availableTo.includes("FREE"))
        .map(({ model, modelName, inputCharLimit, isOpenSource, createdBy }) => ({
        model,
        modelName,
        inputCharLimit,
        createdBy,
...
const generateCompletion = async (messages, config) => {
    if (!config.request)
        config.request = fetch;
    if (!config.vqd)
        config.vqd = await getVqdHash(config.request);
    config.vqd = {
        ...config.vqd,
        client_hashes: await Promise.all(config.vqd.client_hashes.map(async (c) => {
    ...
    const isStream = "streamController" in config &&
        config.streamController instanceof EventEmitter;
    const bodyPayload = {
        messages,
        model: config.model,
        canUseTools: true,
    ...
    const response = await config.request("https://duckduckgo.com/duckchat/v1/chat", payload);
    if (!response.ok) {
        if (isStream)
            config.streamController.emit("error", "Failed get completion");
        throw new Error("Failed get completion");
    }
    const vqd = await parseVqdHash(payload.headers && payload.headers["user-agent"], response.headers.get("X-Vqd-Hash-1") || "");
...
export { EventEmitter, generateCompletion, getModels, getVqdHash };
